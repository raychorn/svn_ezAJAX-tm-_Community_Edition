									...
									if (!!_scopeMember) {
										...
										if (!!dObj) {
											if ( (!!_HASSTYLES) && (_HASSTYLES.toLowerCase() == 'yes') ) {
												var divName = 'div_scopeViewer_' + _scopeMember;
												_html = '<div id="' + divName + '" style="display: inline;">';
												_html += ezBeginTable(_scopeMember, '&bool=' + 'includeDismissButton=true'.ezURLEncode() + '&div=' + 'dismissDiv=' + divName.ezURLEncode() + '&table=' + 'border="1"'.ezURLEncode() + '&tr=' + 'bgcolor="cyan"'.ezURLEncode() + '&td=' + 'align="center"'.ezURLEncode() + '&span=' + 'class="boldPromptTextClass"'.ezURLEncode());
												_html += '<tr><td>';
