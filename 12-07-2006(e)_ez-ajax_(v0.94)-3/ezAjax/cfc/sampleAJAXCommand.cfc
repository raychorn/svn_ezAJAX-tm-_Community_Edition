			...
			try {
				switch (qryStruct.ezCFM) {
					case 'sampleAJAXCommand':
						...
						ezCfDirectory('Request.qDir', ExpandPath('*'), '*');
						if ( (NOT Request.directoryError) AND (IsQuery(Request.qDir)) AND (Request.qDir.recordCount gt 0) ) {
							...
							QuerySetCell(qObj, 'errorMsg', '<font color="red"><b>ezCFDirectory failed to return a Query Object in "#CGI.SCRIPT_NAME#".</b></font>', qObj.recordCount);
