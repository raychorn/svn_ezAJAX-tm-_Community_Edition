...
function _displayContactUsContentIn(oObj, actionID, toEmailAddrs) {
	...
	if (!!oObj) {
		...
		_html += '<input type="Button" id="button_dismiss_contactUs_panel" class="buttonClass" value="[X]" onclick="displayPopUpAtForDiv(_$(stack_displayContactUsContentIn.pop())); return false;">';
		_html += '</td>';
		_html += '</tr>';
		_html += '</table>';
		_html += '</td>';
...
function displayContactUsContentIn(anchorId, id, actionID, toEmailAddrs) {
	var oObj = _$(id);
	if (!!oObj) {
		stack_displayContactUsContentIn.push(id);
		var anchorPos = ezAnchorPosition.get$(anchorId);
		_displayContactUsContentIn(oObj, actionID, toEmailAddrs);
		oObj.style.position = const_absolute_style;
		oObj.style.top = (anchorPos.y + 20) + 'px';
		oObj.style.left = anchorPos.x + 'px';
		oObj.style.zIndex = 1;
		if (!browser_is_ie) {
			oObj.style.top = (ezClientHeight() - 150) + 'px';
		}
