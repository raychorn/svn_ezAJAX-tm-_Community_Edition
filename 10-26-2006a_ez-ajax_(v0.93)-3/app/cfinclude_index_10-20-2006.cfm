...
<cfif 0>
	...
	//		ezAlert('browser_is_ie = [' + browser_is_ie + ']' + ', browser_is_ff = [' + browser_is_ff + ']' + ', browser_is_ns = [' + browser_is_ns + ']'); 
		...
		function populateContentFor(aName) {
			aName = ((typeof aName) == const_string_symbol ? aName : '');
			oAJAXEngine.doAJAX('performPopulateContentFor', 'handlePopulateContentFor', 'aName', aName);
		}
		
		function whatsNewText() {
			_handlePopulateContentFor('');
			return populateContentFor('Whats New');
		}
		
		function _handlePopulateContentFor(someHTML) {
			...
			if (!!qStats) {
				if (!!argsDict) {
					if (!!qData1) {
						...
						_handlePopulateContentFor(html);
					} else {
			...
			<tr>
				...
				<td width="*" align="left">
					<table width="100%" border="0" cellpadding="-1" cellspacing="-1">
						...
						<tr>
							<td valign="top">
								<div id="div_PrimaryContentContainer" style="width: 95%; margin: 5px 5px 5px 5px;"></div>
							</td>
						</tr>
						<tr>
							<td valign="top">
