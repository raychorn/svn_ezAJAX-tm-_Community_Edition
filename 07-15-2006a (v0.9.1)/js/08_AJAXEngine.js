...
ezAJAXEngine.prototype = {
	...
	showAJAXBegins : function(sMsg, sImage, imageHeight) {
		...
		if (!!cObj) {
			...
			var ffadNs = '';
			var ffadN = '/' + ezFirstFolderAfterDomainNameFromHref(docHref) + '/';
			var ffadNi = docHref.toUpperCase().indexOf(ffadN.toUpperCase());
			if (ffadNi > -1) {
				ffadNs = docHref.substr(ffadNi, docHref.length - 1);
				docHref = docHref.ezClipCaselessReplace(ffadNs, ffadN);
			}
			
			if (typeof this.showAJAXBeginsHrefCallback == const_function_symbol) {
				docHref = this.showAJAXBeginsHrefCallback(docHref);
			}
