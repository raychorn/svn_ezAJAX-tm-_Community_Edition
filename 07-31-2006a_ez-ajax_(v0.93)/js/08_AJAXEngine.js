			...
			if (!this.isServerBusy_divName_changed) {
				...
				if (!this.isServerBusy_divName_changed) {
					...
					if (typeof this.ezAJAX_serverBusyCallback == const_function_symbol) {
						try { this.ezAJAX_serverBusyCallback(cObj); } catch(e) {} finally { }
					}
				...
				frObj = _$('iframe_showAJAXBegins');
				if ( (frObj != null) && (!this.isServerBusy_divName_populated) ) {
					try { frObj.contentWindow.document.writeln(resp); } catch(e) { ezAlert('resp = [' + resp + ']'); } finally { }
