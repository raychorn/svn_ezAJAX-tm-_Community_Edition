...
<cfoutput>
	...
	<cfscript>
		...
		logoImage = Request.commonCode.ezClusterizeURL('http://#CGI.SERVER_NAME#/' & Request.scriptFirst & 'app/includes/cfcontent_img.cfm?imageName=../images/Yes02b1a187x171.jpg');
...
//	ezAlert('qObj = [' + qObj + ']');
		...
		function populateSiteContent(num) {
			...
			var iDiv = ezInt(num);
			var oDiv = _$((((iDiv == 101) || (iDiv == 105)) ? 'div_siteContents' : ((iDiv == 102) ? 'div_linesOfCodeContent' : ((iDiv == 103) ? 'div_allTheWorkContent' : ((iDiv == 104) ? 'p_installationProcessStepDesc' : '')))));
			if (!!oDiv) {
				switch (num) {
					case 101.1:
