...
ezTabsObj.openSubTabs = function(oTabsObjId, iTab) {
	...
	if (oTabsObjId <= ezTabsObj.$.length) {
		...
		if (!!oTabsObj) {
			iTab = ((iTab != null) ? iTab : -1);
			oTabsObj.debugThis('ezTabsObj.openSubTabs(' + oTabsObjId + ', ' + iTab + ')');
...
ezTabsObj.hideSubTabs = function(oTabsObjId, iTab) {
	...
	if (oTabsObjId <= ezTabsObj.$.length) {
		...
		if (!!oTabsObj) {
			iTab = ((iTab != null) ? iTab : -1);
			oTabsObj.debugThis('ezTabsObj.hideSubTabs(' + oTabsObjId + ', ' + iTab + ')');
			oTabsObj.hideSubTabs(iTab);
			oTabsObj.isSubTabDismissable = false;
		}
	}
};
...
ezTabsObj._isSubTabDismissable = function(oTabsObjId, bool) {
	var oTabsObj = -1;
	if (oTabsObjId <= ezTabsObj.$.length) {
		oTabsObj = ezTabsObj.$[oTabsObjId];
		if (!!oTabsObj) {
			bool = ((bool == true) ? bool : false);
			oTabsObj.isSubTabDismissable = bool;
			oTabsObj.debugThis('1.1 ezTabsObj._isSubTabDismissable(' + oTabsObjId + ', ' + bool + ')' + ', oTabsObj.timerID = [' + oTabsObj.timerID + ']');
			if (oTabsObj.timerID > -1) {
				clearInterval(oTabsObj.timerID);
				oTabsObj.timerID = -1;
				oTabsObj.debugThis('1.2 clearInterval() :: oTabsObj.timerID = [' + oTabsObj.timerID + ']');
			}
		}
	}
};

ezTabsObj.isSubTabDismissable = function(oTabsObjId, bool) {
	var oTabsObj = -1;
	if (oTabsObjId <= ezTabsObj.$.length) {
		oTabsObj = ezTabsObj.$[oTabsObjId];
		if (!!oTabsObj) {
			bool = ((bool == true) ? bool : false);
			oTabsObj.debugThis('ezTabsObj.isSubTabDismissable(' + oTabsObjId + ', ' + bool + ')');
	//		oTabsObj.timerID = setInterval("ezTabsObj._isSubTabDismissable(" + oTabsObjId + ", " + bool + ");", 2000);
		}
	}
};
...
ezTabsObj.prototype = {
	...
	hideSubTabs : function(iTab) {
		...
		if ( (bool_isActiveTab) && (this.isSubTabDismissable) ) {
		...
		if (iTab > -1) {
			...
			if ((typeof iSubTabs) == const_object_symbol) {
				...
				if (oOuter == null) {
					...
					_html += '<div id="' + tabDivIdOuter + '" onmouseover="var oTable = _$(\'' + tableID + '\'); try { ezTabsObj.debugThis(' + this.id + ', \'onmouseover :: oTable.border = [\' + oTable.border + \'], \'); } catch (e) { }; if ( (this.style.display == const_inline_style) && (!!oTable) && (oTable.border == 0) ) { ezTabsObj.isSubTabDismissable(' + this.id + ', true); oTable.border=1; }; return false;" onmouseout="var oTable = _$(\'' + tableID + '\'); try { ezTabsObj.debugThis(' + this.id + ', \'onmouseout :: oTable.border = [\' + oTable.border + \'], \'); } catch (e) { }; if ( (this.style.display == const_inline_style) && (!!oTable) && (oTable.border == 1) ) { ezTabsObj.hideSubTabs(' + this.id + ', ' + iTab + '); oTable.border=0; }; return false;" style="position: absolute; top: 0px; left: 0px; width: 0px; display: none; cursor:hand; cursor:pointer; z-index: 32767;">';
					_html += this.renderSubTabsFrom(iTab, subTabsAR);
					_html += '</div>';
					if (!!this.oTabSystemTabs) {
						this.oTabSystemTabs.innerHTML += _html;
	//					ezAlert('1.1 addSubTab(' + iTab + ', ' + sTitle + ')\n' + this.oTabSystemTabs.innerHTML);
					}
				} else {
					_html += this.renderSubTabsFrom(iTab, subTabsAR);
					oOuter.innerHTML = _html;
	//				ezAlert('1.2 addSubTab(' + iTab + ', ' + sTitle + ')\n' + oOuter.innerHTML);
				}
			}
		} else {
