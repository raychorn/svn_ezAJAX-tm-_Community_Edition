...
function getElementsWithClass(parent, tagName, klass){
	...
	try {
		var exp = getTokenizedExp(klass,"");
		var collection = (tagName == "*" && parent.all) ? parent.all : parent.getElementsByTagName(tagName);
		
		ezAlert('getElementsWithClass() :: ' + 'parent.id = [' + parent.id + ']');
		ezAlert('getElementsWithClass() :: ' + 'tagName = [' + tagName + ']');
		ezAlert('getElementsWithClass() :: ' + 'klass = [' + klass + ']');
		ezAlert('getElementsWithClass() :: ' + 'collection.length = [' + collection.length + ']');
		ezAlert('getElementsWithClass() :: ' + 'exp = [' + exp + ']');

		for(var i = 0, counter = 0; i < collection.length; i++) {
			ezAlert('getElementsWithClass() :: ' + 'collection[' + i + '].className = [' + collection[i].className + ']' + ((!!collection[i].id) ? 'getElementsWithClass() :: ' + 'collection[' + i + '].id = [' + collection[i].id + ']' : ''));
			if(exp.test(collection[i].className))
				returnedCollection[counter++] = collection[i];
		}
		return returnedCollection;
