...
function tabInit() {
	...
	ezAlert('tabInit() :: ' + 'Browser.isSupported() = [' + Browser.isSupported() + ']' + ', window.tabInited = [' + window.tabInited + ']');

	if(!Browser.isSupported() || window.tabInited) return;
	
	var oTabSystemContainer = _$('TabSystemContainer');

	ezAlert('tabInit() :: ' + 'oTabSystem1 = [' + oTabSystemContainer + ']');

	var tabsDivs = getElementsWithClass(((!!oTabSystemContainer) ? oTabSystemContainer : document.body), "div".toUpperCase(), "tabs");
	
	ezAlert('tabInit() :: ' + 'tabsDivs.length = [' + tabsDivs.length + ']');
	...
	ezAlert('tabInit() :: ' + 'list.length = [' + list.length + ']');
