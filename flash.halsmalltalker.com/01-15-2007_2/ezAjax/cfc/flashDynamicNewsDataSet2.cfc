<cfcomponent displayname="flashDynamicNewsDataSet2" output="No" extends="userDefinedAJAXFunctions">
	<cfscript>
		function _userDefinedAJAXFunctions(qryStruct) {
			...
			try {
				switch (qryStruct.ezCFM) {
					case 'flashDynamicNewsDataSet2':
						qObj = QueryNew('ID, status'); // You may add Query Elements to this Query as needed...
						QueryAddRow(qObj, 1);
						QuerySetCell(qObj, 'ID', qObj.recordCount, qObj.recordCount);
						QuerySetCell(qObj, 'status', 'Dataset has been created.', qObj.recordCount);
