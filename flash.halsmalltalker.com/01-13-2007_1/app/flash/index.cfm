...
//		oAJAXEngine.isXmlHttpPreferred = true;
		...
		function handle_onClick_buildFlashAJAXData2(btnObj) {
			...
			switch (verb) {
				case 'Remove':
					alert('verb = [' + verb + ']' + ', row = [' + row + ']' + ', col = [' + col + ']');
					break;
				case 'Add':
					var oInputCat = _$('input_FlashAJAXData2_' + row + '_1')
					var oInputSubCat = _$('input_FlashAJAXData2_' + row + '_2')
					var oInputUrl = _$('input_FlashAJAXData2_' + row + '_3')
	...
	//			}
		...
		function buildFlashAJAXData2() {
			var divName = 'div_Flash5_DynamicNewsDemo_with_AJAX';
			var _html = '';
			...
			_html += '<td><input id="input_FlashAJAXData2_1_1" class="textClass" size="12" maxlength="16" title="Enter the Category here - The Category is the main heading for the group of SubCategories; you may think of the Category as if it were the Main Menu item and the SubCategory as if it were the SubMenu item."></td>';
			_html += '<td><input id="input_FlashAJAXData2_1_2" class="textClass" size="12" maxlength="16" title="Enter the SubCategory here - The SubCategory is the heading for the URL; you may think of the SubCategory as if it were the SubMenu item that falls under each Category which is the Main Menu item."></td>';
			_html += '<td><input id="input_FlashAJAXData2_1_3" class="textClass" size="12" maxlength="16" title="Enter the URL here - This URL can be any valid Internet URL."></td>';
			_html += '</tr>';
			_html += ezEndTable();
		...
		function getTab4SubTab11Content() {
			var _html = '';
			var headerFontSize = ((!ezAJAXEngine.browser_is_ie) ? '15px' : '13px');
			var bodyFontSize = (parseInt(headerFontSize) - 2) + 'px';
			_html += ((!ezAJAXEngine.browser_is_ie) ? '<div style="margin-top: 50px;">' : '');
		...
		function getTabbedContent() {
			...
			oTabSystem1.onActivateTabsCallback = function(iTab) { clearContentFromTabs(this); var _id = this.contentsCollection[this.tabsDict.getValueFor(iTab) - 1]; var oDiv = _$(_id); if (!!oDiv) { var s = ''; var xFunc = -1; sFunc = 'getTab' + iTab + 'Content'; try { xFunc = eval(sFunc); s = xFunc(); } catch (e) { ezAlertError(ezErrorExplainer(e)); }; if (s.length > 0) { oDiv.innerHTML = s; }; }; };
