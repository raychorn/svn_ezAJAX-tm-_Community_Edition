...
//		oAJAXEngine.isXmlHttpPreferred = true;
		...
		function getTabbedContent() {
			...
			oTabSystem1.onActivateTabsCallback = function(iTab) { var _id = this.contentsCollection[this.tabsDict.getValueFor(iTab) - 1]; var oDiv = _$(_id); if (!!oDiv) { var s = ''; switch (iTab) { case 2: s = getTab2Content(); break; }; /*ezAlert('iTab = [' + iTab + ']' + ', _id = [' + _id + ']' + ', oDiv = [' + oDiv + ']' + ', s.length = [' + s.length + ']' + ', this.isSubTabClicked = [' + this.isSubTabClicked + ']');*/ if (s.length > 0) { oDiv.innerHTML = s; }; }; };
			oTabSystem1.onShowSubTabsCallback = function(iTab, oStyle, oParent, anchorPos, anchorPosL, anchorPosR){ var w = (anchorPosR.x - anchorPosL.x) + 20; oStyle.top = ezElementPositonY(oParent) - 70; oStyle.left = ezElementPositonX(oParent); oStyle.width = w; oStyle.zIndex = 32760; /*ezAlert('oTabSystem1.onShowSubTabsCallback :: ' + 'iTab = [' + iTab + ']' + ', oStyle = [' + ezStyle2String(oStyle) + ']' + ', oParent.id = [' + oParent.id + ']' + ', anchorPos = [' + anchorPos + ']' + ', anchorPosL = [' + anchorPosL + ']' + ', anchorPosR = [' + anchorPosR + ']');*/ };
			oTabSystem1.onClickSubTabCallback = function(iSubTab, iTab){ var _id = this.contentsCollection[this.subTabsContentDict.getValueFor(iSubTab) - 1]; /*ezAlert('oTabSystem1.onClickSubTabCallback :: ' + 'iSubTab = [' + iSubTab + ']' + ', iTab = [' + iTab + ']' + ', _id = [' + _id + ']');*/ this.isSubTabClicked = true; var oDiv = _$(_id); if (!!oDiv) { var s = ''; switch (iSubTab) { case 1: s = getTab1SubTab1Content(); break; case 2: s = getTab1SubTab2Content(); break; }; if (s.length > 0) { oDiv.innerHTML = s; }; }; };
			...
			iSubTab = oTabSystem1.addSubTab(oTab, 'Flash 4 Menu Demo');
			iContent = oTabSystem1.addContentForSubTab(iSubTab, '')

			iSubTab = oTabSystem1.addSubTab(oTab, 'Flash 4 NavBar Demo');
			iContent = oTabSystem1.addContentForSubTab(iSubTab, '')
