...
<cfif (NOT debugJavaScriptPackager)>
			...
			function displayObjectsBrowserGUI() {
				...
				function displayMetaDataRecord(_ri, _dict, _rowCntName) {
					...
					if (_TYPE_NAME.toUpperCase().indexOf(' identity'.toUpperCase()) == -1) {
						...
						} catch (e) { ezAlertError('1.3.1.2\n' + ezErrorExplainer(e)); };
					}
				};

				ezAlert('displayObjectsBrowserGUI 1.0 :: ' + 'dObj = [' + dObj + ']' + ', cObj = [' + cObj + ']');
				if ( (!!dObj) && (!!cObj) ) {
					...
					ezAlert('displayObjectsBrowserGUI 1.1 :: ' + 'sel = [' + sel + ']');
					if (sel > -1) {
						var aDict = ezGeonosisObj.searchInstancesForClassName(cObj.options[sel].text);
						ezAlert('displayObjectsBrowserGUI 1.2 :: ' + 'aDict = [' + aDict + ']');
						...
						ezAlert('displayObjectsBrowserGUI 1.3 :: ' + '_OBJECTNAME = [' + _OBJECTNAME + ']' + ', _ID = [' + _ID + ']');
	...
	//					ezAlert('displayObjectsBrowser :: 1.1 ' + 'dataDict = [' + dataDict + ']\n' + '============================================================\n');
						var qSchema_OBJECTSMETDATARECORD = dataDict.getValueFor('OBJECTSMETDATARECORD');
	//					ezAlert('displayObjectsBrowser :: 1.1.1 ' + 'qSchema_OBJECTSMETDATARECORD = [' + qSchema_OBJECTSMETDATARECORD + ']\n' + '============================================================\n');
	...
	//					ezAlert('displayObjectsBrowser :: 1.2.0 ' + 'qSchema_SCOPESCONTENTQUERY = [' + qSchema_SCOPESCONTENTQUERY + ']\n' + '============================================================\n');
						if (qSchema_SCOPESCONTENTQUERY > -1) {
							var qScopeContent = qObj.named('qData' + qSchema_SCOPESCONTENTQUERY)
	//						ezAlert('displayObjectsBrowser :: 1.2.1 ' + 'qScopeContent = [' + qScopeContent + ']\n' + '============================================================\n');
							if (!!qScopeContent) {
								dataDict = -1;
	...
	//							ezAlert('oGeonosis ##' + oGeonosis.id + '\n\noGeonosis.qData = [' + oGeonosis.qData + ']' + '\n\noGeonosis.qAttrs = [' + oGeonosis.qAttrs + ']');
	//							ezAlert('==============================================================\n\n');
							}
						} catch (e) { ezAlertError('999\n' + ezErrorExplainer(e)); };

	//					ezAlert('displayObjectsBrowser :: 1.2 ' + 'qObj.names.length = [' + qObj.names.length + ']' + ', qObj.names = [' + qObj.names + ']' + ', argsDict = [' + argsDict + ']\n' + '============================================================\n');
					...
					if (!bool_isAnyErrorRecords) {
						...
						switch (_ezCFM) {
							...
							case 'performGetGeonosisObjects':
								...
								} catch (e) { ezAlert('211.1 (' + _ezCFM + ') realQueryObjectNames = [' + realQueryObjectNames + ']\n' + ezErrorExplainer(e)); };
