<cfcomponent displayname="performPopulateContentFor" output="No" extends="userDefinedAJAXFunctions">
	...
	<cfsavecontent variable="content_106_1_onclick_btn_manageLicenses_Submit">
		<cfif 0>
			<cfoutput>function _handle_DynamicCodeInitiator_handleGetRegistrationData(_emailAddress, _serverName) { _emailAddress = (((typeof _emailAddress) == const_string_symbol) ? _emailAddress : ' '); _serverName = (((typeof _serverName) == const_string_symbol) ? _serverName : ' '); oAJAXEngine.doAJAX('performGetRegistrationData', 'handleGetRegistrationData', 'emailAddress', _emailAddress, 'serverName', _serverName); };  function handle_DynamicCodeInitiator_handleGetRegistrationData() { if (cache_processDynamicCodeTracker[handle_DynamicCodeInitiator_handleGetRegistrationData] != null) { clearInterval(cache_processDynamicCodeTracker[handle_DynamicCodeInitiator_handleGetRegistrationData]); cache_processDynamicCodeTracker[handle_DynamicCodeInitiator_handleGetRegistrationData] = null; stack_processDynamicCodeTracker.pop(); }; try { _handle_DynamicCodeInitiator_handleGetRegistrationData(oInput1.value, oInput2.value); } catch (e) { ezAlertError(ezErrorExplainer(e)); }; }; function boolIsModuleLoaded_handleGetRegistrationData() { return true; }; var oInput1 = _$('manageLicenses_emailAddress'); var oInput2 = _$('manageLicenses_serverName'); if ( (!!oInput1) && (!!oInput2) ) { if ( (oInput1.value.length > 0) && (oInput2.value.length > 0) ) { oGuiActions.pushButton(this.id); oGuiActions.pushButton(oInput1.id); oGuiActions.pushButton(oInput2.id); if (global_previouslyLoadedObjects.indexOf('/app/js/handleGetRegistrationData.js') != -1) { try { _handle_DynamicCodeInitiator_handleGetRegistrationData(oInput1.value, oInput2.value); } catch (e) { ezAlertError(ezErrorExplainer(e)); }; } else { ezDynamicObjectLoader(ezAJAXEngine.$[0]._url + '/app/js/handleGetRegistrationData.js'); processDynamicCodeTracker(handle_DynamicCodeInitiator_handleGetRegistrationData, boolIsModuleLoaded_handleGetRegistrationData); } } else { alert('Missing values for EMail Address and/or Server Name.'); } } else { alert('ERROR: Programming Error - Unable to fetch the value for \'emailAddress\' and/or \'serverName\'.'); }</cfoutput>
		<cfelse>
			<cfoutput>function _handle_DynamicCodeInitiator_handleGetRegistrationData(_emailAddress, _serverName) { _emailAddress = (((typeof _emailAddress) == const_string_symbol) ? _emailAddress : ' '); _serverName = (((typeof _serverName) == const_string_symbol) ? _serverName : ' '); oAJAXEngine.doAJAX('performGetRegistrationData', 'handleGetRegistrationData', 'emailAddress', _emailAddress, 'serverName', _serverName); };  function handle_DynamicCodeInitiator_handleGetRegistrationData() { if (cache_processDynamicCodeTracker[handle_DynamicCodeInitiator_handleGetRegistrationData] != null) { clearInterval(cache_processDynamicCodeTracker[handle_DynamicCodeInitiator_handleGetRegistrationData]); cache_processDynamicCodeTracker[handle_DynamicCodeInitiator_handleGetRegistrationData] = null; stack_processDynamicCodeTracker.pop(); }; try { _handle_DynamicCodeInitiator_handleGetRegistrationData(oInput1.value, oInput2.value); } catch (e) { ezAlertError(ezErrorExplainer(e)); }; }; function boolIsModuleLoaded_handleGetRegistrationData() { return true; }; var oInput1 = _$('manageLicenses_emailAddress'); var oInput2 = _$('manageLicenses_serverName'); if ( (!!oInput1) && (!!oInput2) ) { if ( (oInput1.value.length > 0) && (oInput2.value.length > 0) ) { oGuiActions.pushButton(this.id); oGuiActions.pushButton(oInput1.id); oGuiActions.pushButton(oInput2.id); if (global_previouslyLoadedObjects.indexOf('&fname=js\\handleGetRegistrationData.js') != -1) { try { _handle_DynamicCodeInitiator_handleGetRegistrationData(oInput1.value, oInput2.value); } catch (e) { ezAlertError(ezErrorExplainer(e)); }; } else { ezDynamicObjectLoader(ezAJAXEngine.$[0]._url + '/app/loader.cfm?out=1&dsr=1&fname=js\\handleGetRegistrationData.js'); processDynamicCodeTracker(handle_DynamicCodeInitiator_handleGetRegistrationData, boolIsModuleLoaded_handleGetRegistrationData); } } else { alert('Missing values for EMail Address and/or Server Name.'); } } else { alert('ERROR: Programming Error - Unable to fetch the value for \'emailAddress\' and/or \'serverName\'.'); }</cfoutput>
		</cfif>
	</cfsavecontent>
										...
										<table width="100%" cellpadding="-1" cellspacing="-1">
											...
											<tr>
												<td align="left">
													...
													&nbsp;<button id="btn_manageLicenses_Submit" type="button" disabled class="buttonMenuClass<cfif (ezIsBrowserFF() OR ezIsBrowserNS())>FF</cfif>" tabindex="3" onclick="#content_106_1_onclick_btn_manageLicenses_Submit# return false;">[Retrieve Runtime Licenses]</button>&nbsp;
													<cfif 0>
	...
	<cfsavecontent variable="content_107_1a">
		<cfoutput>
			<div id="div_WhatsIsContentContainer" style="margin: 5px 5px 5px 5px;">
				...
				<p align="justify" class="normalPrompt"><b>ezAJAX Version 0.95 will fully support XHTML 1.0 as well as HTML 4.1 fully configurable and fully functional.  ezAJAX Version 0.95 will feature an optimized JavaScript loader to help minimize the client footprint.  ezAJAX Version 0.95 will also feature an AJAX powered Server Scope Debug Panel to allow developers to interactively view the following server-side ColdFusion Scopes: Application, Session, CGI and Request.  ezAJAX Version 0.95 will also feature automatic handling of situations in which one ezAJAX Server Command is in-process at the same time a new Server Command is presented to the ezAJAXEngine; previous version simply accepted the new command while discarding the other command.  Very soon ezAJAX Developers will be able to automatically handle asynchronous AJAX Server Commands without having to be the least bit concerned about when each Server Command is presented for processing.</b></p>
