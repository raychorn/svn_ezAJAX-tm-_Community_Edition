<cfcomponent displayname="performPopulateContentFor" output="No" extends="userDefinedAJAXFunctions">
	...
	<cfscript>
		...
		QueryAddRow(qFeatures, 1);
		QuerySetCell(qFeatures, 'id', qFeatures.recordCount, qFeatures.recordCount);
		QuerySetCell(qFeatures, 'sFeature', "<b>10000+ Lines of Robust Reusable JavaScript Code you can Redistribute with YOUR Application.</b>", qFeatures.recordCount);
		QuerySetCell(qFeatures, 'sCheckCE', "<b>X</b>", qFeatures.recordCount);
		QuerySetCell(qFeatures, 'sCheckEE', "<b>X</b>", qFeatures.recordCount);
		QueryAddRow(qFeatures, 1);
		QuerySetCell(qFeatures, 'id', qFeatures.recordCount, qFeatures.recordCount);
		...
		<cfoutput>
			<div id="div_WhatsNewContentContainer" style="margin: 5px 5px 5px 5px;">
				<table width="100%" cellpadding="-1" cellspacing="-1">
					...
					<tr>
						<td>
							<p align="justify" class="normalPrompt"><b>ezAJAX Version 0.93 is now ready for downloads and is completely compatible with <a href="" onclick="try { sampleAppsText(); } catch(e) { } finally { }; return false;">Dojo 0.3.1</a></b>&nbsp;
							You can <a href="" onclick="#cf_urlDojoMailSample#; return false;">view our Dojo Sample - the Mail Sample Application</a> as taken from <a href="" onclick="try { sampleAppsText(); } catch(e) { } finally { }; return false;">Dojo 0.3.1</a>.<cfif (ezIsBrowserIE())>&nbsp;<cfelse>&nbsp;&nbsp;</cfif>
							...
							<p align="justify" class="normalPrompt"><b>The next version of ezAJAX will completely support JSON.  For those of you who may not be aware of what JSON is (as if this requires any explanation) JSON is a nice compact method for transmitting data from place to place using JavaScript.  The problem with JSON is that JSON does not typically provide a Schema for the data one wishes to convey from the Server to the client.  ezAJAX provides JSON what it has been missing by automatically converting JSON data blocks into ezAJAX Query Objects so that Query of Queries can be executed from the client's perspective.  Client-side Query of Queries can execute much faster than server-side Queries and this alone can enable your ezAJAX Apps to run much faster than similar AJAX apps that were constructed using some other Framework other than ezAJAX.</b></p>
							<p align="justify" class="normalPrompt"><b>The next version of ezAJAX will completely support all Yahoo Web 2.0 API's that use JSON.  We have published an ezAJAX Sample Applet that interfaces with one of the Yahoo Web 2.0 API's. You can also expect ezAJAX to completely support the same cross-domain AJAX method that Yahoo is using internally at Yahoo.  Yahoo recently contacted our development staff to have our developers create a cross-domain AJAX method for them and soon ezAJAX will use this same cross-domain AJAX method.  In addition to the iFrame and xmlHttpRequest AJAX methods you will also be able to use the cross-domain AJAX method we programmed for Yahoo.</b></p>
						</td>
					</tr>
					<tr>
						<td>
						...
						if (IsDefined("qryStruct.namedArgs.aName")) {
							...
							switch (qryStruct.namedArgs.aName) {
								...
								case 'Yahoo Search V2':
									QuerySetCell(qObj, 'CONTENT', content_YahooSearchV2, qObj.recordCount);
								break;
