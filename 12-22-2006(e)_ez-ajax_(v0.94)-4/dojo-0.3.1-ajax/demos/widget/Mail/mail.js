...
function displayList(){
    this.update = function(message) {
		...
		var nodeName = clickedTreeNode.title.replace(" ","");
		var url = "Mail/"+ nodeName + ".html";
		var listPane = dojo.widget.getWidgetById("listPane");
		if (cache_nodeCache[nodeName]) {
			listPane.setContent(cache_nodeCache[nodeName]);
			cache_nodeCache[nodeName] = null;
		} else {
			listPane.setUrl(url);
		}
    };
