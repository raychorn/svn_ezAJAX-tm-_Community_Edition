	...
	_onActivateTabsBrowserCallback : function(iTab) {
		...
		if (!!oDiv) {
			if (!ezTabsObj.isFirstTabActivated) {
				oAJAXEngine.doAJAX('performClickMenuTab', 'ezTabsObj.handlePerformClickMenuTab', 'iTab', iTab, 'id', this.id);
			} else {
		...
		if ( (iTab > 0) && (iTab <= this.tabsCollection.length) ) {
			...
			if (!this.isIgnoreTabActivation) {
				if ( (this.isTabClickable) && (typeof this.onActivateTabsCallback == const_function_symbol) ) {
					try { bool_okayToActivate = this.onActivateTabsCallback(iTab); } catch(e) { ezAlertError('oTabsObj.activateTab ::\n' + ezErrorExplainer(e));};
					bool_okayToActivate = ((bool_okayToActivate == null) ? true : bool_okayToActivate);
				}
				if (bool_okayToActivate) {
