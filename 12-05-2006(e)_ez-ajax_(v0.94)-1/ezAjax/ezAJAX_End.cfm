...
<cfif (bool_using_xmlHttpRequest)>
	...
	<cfelse>
		<cfoutput>
			...
			try { server_response_queue = []; server_response_queue.push(_#___jsName___#); if (oAJAXEngine) { oAJAXEngine.isPacketJSON = true; oAJAXEngine.receivePacket(server_response_queue); ezAJAXEngine.receiveJSONPacket(_#___jsName___#, #Request.qryStruct.ezCallBack#); } } catch (e) { ezAlertError('JSON.2 ::\n' + ezErrorExplainer(e)); };
			/* EOF CFAJAX */
		</cfoutput>
	</cfif>
<cfelse>
