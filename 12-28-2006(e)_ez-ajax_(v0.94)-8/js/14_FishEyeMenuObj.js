...
ezFishEyeMenuObj.prototype = {
	...
	addIcon : function(imgUrl, onClickFunc, aCaption) {
		...
		if (!!oDiv) {
			oDiv.innerHTML += '<div id="div_fishEyeIcon_' + this.id + '_' + this.iconsDict.length() + '"><table><tr><td><img id="img_fishEyeIcon_' + this.id + '_' + this.iconsDict.length() + '" width="' + this.iWidth + '" height="' + this.iHeight + '" src="' + imgUrl + '" height="' + (this.height / 2) + '" border="0" title="' + aCaption + '" onmouseover="this.height = Math.max(this.height * 2, ' + this.height + '); this.width = this.height * 2; return false;" onmouseout="this.height = (' + (this.height / 2) + '); this.width = this.height; return false;" onclick="ezFishEyeMenuObj.onClick(this.id, \'' + imgUrl + '\'); return false;"></td></tr><tr><td><span class="textClass">' + aCaption + '</span></td></tr></table></div>';
		}
		return this;
	},
	horizontal : function() {
