...
function ezObjectExplainer(obj, bool_includeFuncs, _cnt) {
	...
	var i = -1;
	var a = [];
	var cnt = ((_cnt == null) ? '1' : _cnt.toString() + '.1');
	...
	_db = '';
	if ((typeof obj.toString) == const_function_symbol) {
		_db = obj.toString();
	} else {
			...
			if (obj.length != null) {
			    for (i = 0; i < obj.length; i++) {
					if ( ( (bool_includeFuncs) && ((typeof obj[i]) == const_function_symbol) ) || ( (!bool_includeFuncs) && ((typeof obj[i]) != const_function_symbol) ) ) {
						a.push(((isCntComplex(cnt)) ? '' : incrementCnt(cnt) + ' :: ') + '[' + obj[i] + ']');
					}
			    }
			} else {
				for (m in obj) {
					if ((typeof obj[m]) == const_object_symbol) {
						a.push(incrementCnt(cnt) + ' :: ' + m + ' = [' + ezObjectExplainer(obj[m], bool_includeFuncs, cnt) + ']');
					} else if ( ( (bool_includeFuncs) && ((typeof obj[m]) == const_function_symbol) ) || ( (!bool_includeFuncs) && ((typeof obj[m]) != const_function_symbol) ) ) {
						a.push(((isCntComplex(cnt)) ? '' : incrementCnt(cnt) + ' :: ') + m + ' = [' + obj[m] + ']');
					}
				}
			}
