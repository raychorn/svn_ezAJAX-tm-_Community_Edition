					...
					switch (Request.qryStruct.ezCFM) {
						...
						default:
							...
							if ( (NOT IsDefined("Request.qryData")) OR (NOT IsArray(Request.qryData)) OR (ArrayLen(Request.qryData) eq 0) ) {
								...
								QueryAddRow(qObj, 1);
								QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
								QuerySetCell(qObj, 'errorMsg', 'ezAJAXEngine Server Error - No Server Command implementer was detected.&nbsp;&nbsp;Double-check the userDefinedAJAXFunctions.cfc file to ensure you have implemented the command (#Request.qryStruct.ezCFM#).', qObj.recordCount);
