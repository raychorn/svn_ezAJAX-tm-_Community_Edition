			...
			try {
				switch (qryStruct.ezCFM) {
					case 'performEmailNewsletters':
						_sql_statement = "SELECT UserNames.username, Newsletters.dtSent FROM UserNames LEFT OUTER JOIN Newsletters ON UserNames.id = Newsletters.uid WHERE (Newsletters.dtSent IS NULL) OR (DATEDIFF(d, GETDATE(), Newsletters.dtSent) > 30) ORDER BY UserNames.username";
