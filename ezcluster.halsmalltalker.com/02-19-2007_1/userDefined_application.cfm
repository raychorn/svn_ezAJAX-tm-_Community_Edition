...
<cfscript>
	...
	boolIsServerProduction = (FindNoCase(cf_productionServerToken, CGI.SERVER_NAME) gt 0);  // this boolean value tells the framework when your app is running in Production mode versus Development mode - Development mode displays the debugging menu whereas Production mode does not...
	bool_isServerProduction = ( (FindNoCase("192.168.", CGI.REMOTE_ADDR) eq 0) AND (FindNoCase("71.133.203.2", CGI.REMOTE_ADDR) eq 0) );  // this boolean value tells the framework when your app is running in Production mode versus Development mode - //	bool_isServerProduction = true;
</cfscript>
