...
<cfoutput>
		...
		<script language="JavaScript1.2" type="text/javascript">
			function changeServerStatus(sObj, serverNum) {
				if ( (!!sObj) && (!!sObj.options) && (sObj.selectedIndex > -1) ) {
					...
					document.location.href = '#CGI.SCRIPT_NAME#?serverNum=' + serverNum + '&serverStatus=' + sObj.options[sObj.selectedIndex].value + '&nocache=' + parent.ezUUID$() + '&debugMode=#debugMode#';
				}
			}
			
			function changeClusterMethod(sObj) {
				if ( (!!sObj) && (!!sObj.options) && (sObj.selectedIndex > -1) ) {
					document.location.href = '#CGI.SCRIPT_NAME#?clusterMethod=' + sObj.options[sObj.selectedIndex].value + '&nocache=' + parent.ezUUID$() + '&debugMode=#debugMode#';
				}
			}
		</script>
	</head>
	...
	<script language="JavaScript1.2" type="text/javascript">
		function refreshServer(iNum) {
			...
			var oObj = parent._$('iframe_server' + iNum);
			if (!!oObj) {
				oObj.src = 'clusterStatusInterface.cfm?debugMode=0&serverNum=' + iNum + '&nocache=' + parent.ezUUID$();
			}
		}
		...
		function refreshTimerForServer(iNum) {
			iNum = ((typeof iNum == 'number') ? iNum : 1);
			var oObj = parent._$('div_timer_server' + iNum);
			if (!!oObj) {
		...
		function refreshTimerForCluster() {
			var oObj = parent._$('div_timer_cluster');
			if (!!oObj) {
