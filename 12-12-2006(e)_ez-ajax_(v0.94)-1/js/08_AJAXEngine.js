...
ezAJAXEngine._initXmlHttpRequest = function(sSearch) {
	function getURLQueryString(s) {
		s = (((typeof s) == const_string_symbol) ? s : '');
		var ar = s.split('?');
		return ar[ar.length - 1];
	}
	
	sSearch = (((typeof sSearch) == const_string_symbol) ? sSearch.ezURLDecode() : '');
//	var _search = getURLQueryString(sSearch);
	var _search = sSearch;
	var i = _search.indexOf('?') + 1;
	var j = _search.indexOf('&');
	var idStr = _search.substring(i,j);

	ezAlert('1.1 ezAJAXEngine._initXmlHttpRequest :: _search = [' + _search + ']\n');
	ezAlert('1.2 ezAJAXEngine._initXmlHttpRequest :: i = [' + i + ']\n');
	ezAlert('1.3 ezAJAXEngine._initXmlHttpRequest :: j = [' + j + ']\n');
	ezAlert('1.4 ezAJAXEngine._initXmlHttpRequest :: idStr = [' + idStr + ']\n');
	
	var _id = parseInt(idStr);
	var oInstance = ezAJAXEngine.$[_id];
	ezAlert('1.5 ezAJAXEngine._initXmlHttpRequest :: _id = [' + _id + ']\n');
	ezAlert('1.6 ezAJAXEngine._initXmlHttpRequest :: oInstance = [' + oInstance + ']\n');
	if (!!oInstance) {
		var _url = _search.substring(j + 1, _search.length);
		ezAlert('2.1 ezAJAXEngine._initXmlHttpRequest :: _url = [' + _url + ']\n');
...
ezAJAXEngine.prototype = {
	...
	initXmlHttpRequest : function(url) {
		aObj = _$(this.ajaxID);
		if (!!aObj) {
//			ezAlert('initXmlHttpRequest :: ' + 'url = [' + url + ']\n' + 'aObj.src = [' + aObj.src + ']');
