				...
				if (!!qData1) {
					...
					if (!!oParms) {
						...
						if ( (!ezAJAXEngine.isCommunityEdition) && (nRecs > 1) ) {
							...
							var qDataN = -1;
							for (i = 1; i <= nRecs; i++) {
								qDataN = qObj.named('qData' + i);
								if (!!qDataN) {
									qDataN.iterateRecObjs(anyErrorRecords);
									ar.push(qDataN);
								}
							}
							qData1 = ar; // this becomes an array to a series of Query Objects rather than a single Query Object...
						} else {
...
ezAJAXEngine.prototype = {
	...
	//	ezAlert('1.0 sendPacket :: this.sent = [' + this.sent + ']');
		this.sent = '&___jsName___=' + ezAJAXEngine.js_global_varName + packet;
	//	ezAlert('1.1 sendPacket :: this.sent = [' + this.sent + ']');
