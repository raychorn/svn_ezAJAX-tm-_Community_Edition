				...
				switch (qryStruct.ezCFM) {
					case 'performOptInAction':
						if ( (IsDefined("qryStruct.namedArgs.emailAddress")) AND (Len(qryStruct.namedArgs.emailAddress) gt 0) ) { 
							_sql_statement = "INSERT INTO OptInNewsletters (emailAddress) VALUES ('#ezFilterQuotesForSQL(qryStruct.namedArgs.EmailAddress)#')";
					...
					case 'performEmailNewsletters':
						_sql_statement = "SELECT UserNames.username, Newsletters.dtSent FROM UserNames LEFT OUTER JOIN Newsletters ON UserNames.id = Newsletters.uid WHERE (Newsletters.dtSent IS NULL) AND ( (Newsletters.removed IS NULL) OR (Newsletters.removed = 0) ) AND (DATEDIFF(d, GETDATE(), Newsletters.dtSent) > 30) ORDER BY UserNames.username";
					//	_dsn = Request.ezAJAX_DSN;
						_dsn = Request.ezAJAX_Newsletters_DSN;
						ezExecSQL('Request.qGetNewsletterSubscribers', _dsn, _sql_statement);
						if (NOT Request.dbError) {
							if (Request.qGetNewsletterSubscribers.recordCount gt 0) {
								folderName = DateFormat(Now(), 'mmmm') & DateFormat(Now(), 'yyyy');
							...
							if (NOT Request.dbError) {
								if (Request.qGetRegistrationData.recordCount gt 0) {
									...
									if (NOT Request.dbError) {
										...
										ezRegisterQueryFromAJAX(Request.qGetRegistrationData); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
									} else {
										qObj = QueryNew('id, errorMsg, moreErrorMsg, explainError, isPKviolation');
										QueryAddRow(qObj, 1);
										QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
