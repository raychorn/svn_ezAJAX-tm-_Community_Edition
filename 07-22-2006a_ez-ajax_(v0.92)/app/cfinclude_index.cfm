					...
					if (!!qData1) {
						...
						if (!isBoolUpdate) {
							...
							if (!!dObj) {
								...
								podLayoutContainerStylesCallBack = -1;
								dObj.innerHTML += ((jsBool_isCurrentBlogDefault) ? rapContent : '') + ((jsBool_isCurrentBlogDefault) ? paypalPod() : '') + sampleAJAXAppsPod();
...
//	ezAlert('qObj = [' + qObj + ']');
		...
		function populateSiteContent(num) {
			...
			var iDiv = ezInt(num);
			var oDiv = _$((((iDiv == 101) || (iDiv == 105) || (iDiv == 106)) ? 'div_siteContents' : ((iDiv == 102) ? 'div_linesOfCodeContent' : ((iDiv == 103) ? 'div_allTheWorkContent' : ((iDiv == 104) ? 'p_installationProcessStepDesc' : '')))));
			if (!!oDiv) {
				switch (num) {
					case 101.1:
