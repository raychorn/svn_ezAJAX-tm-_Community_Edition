...
<cfoutput>
	...
	<cfsavecontent variable="jsCodeStore0">
		...
		if (bool_useAbsoluteServerBusyPositioning) {
			...
			oAJAXEngine.ezAJAX_serverBusyCallback = function (cObj, resp) { var oO = _$('iframe_showAJAXBegins'); if ( (!!oO) && (!this.isServerBusy_divName_populated) ) { oO.contentWindow.document.writeln(resp); this.isServerBusy_divName_populated = true; }; if (oO.style.display == const_none_style) { oO.style.display = const_inline_style; }; resizeOuterContentWrapper(ezClientWidth()); };
		}
	...
	<cfscript>
		...
		_urlDojoMailSample = 'http://#CGI.SERVER_NAME#/dojo-0.4.1-ajax/demos/widget/mail.html';
	...
	<cfsavecontent variable="jsCodeStore1">
		...
		function popUpWindowForDojoMailSample() {
			#cf_urlDojoMailSample#;
		}
		...
		function sampleWeb20APIText(pageId) {
			pageId = ((pageId == null) ? '' : parseInt(pageId.toString()));
			_handlePopulateContentFor('', 'div_PrimaryContentContainer');
			return populateContentFor('Web20API' + pageId, 'div_PrimaryContentContainer');
		}
			...
			<tr>
				...
				<td width="*" align="left">
					<table width="100%" border="0" cellpadding="-1" cellspacing="-1">
						<tr>
							<td width="100%" valign="top">
								<table width="100%" cellpadding="-1" cellspacing="-1" class="bannerWhiteTable">
									...
									<tr>
										<td align="center">
											<h3 align="center"><b>This is the <U>MUST-HAVE</U> Web 2.0 Product of #Year(Now())# !</b></h3>
										</td>
									</tr>
									<tr>
										<td align="center">
											<font color="blue"><b>Limited Time Bundle Offer: The first 10 ezAJAX Enterprise Edition Perpetual Runtime License Registrations, <i>every month</i>, will get a FREE License for ColdFusion MX 7 Server.</b></font>
										</td>
									</tr>
								</table>
							</td>
						...
						<tr>
							<td valign="top">
								<div id="div_PrimaryContentContainer" style="width: 95%;"><cfif (Len(CGI.PATH_INFO) gt 0)>#getUsersContent(ListFirst(CGI.PATH_INFO, Left(CGI.PATH_INFO, 1)))#</cfif></div>
							</td>
						</tr>
						<tr>
							<td valign="top">
	...
	<cfif (Len(CGI.PATH_INFO) eq 0)>
		<script language="JavaScript1.2" type="text/javascript">
			handleOnClickSsmItemsByName(ssmItems[1][0]);whatsNewText();
		</script>
	</cfif>
