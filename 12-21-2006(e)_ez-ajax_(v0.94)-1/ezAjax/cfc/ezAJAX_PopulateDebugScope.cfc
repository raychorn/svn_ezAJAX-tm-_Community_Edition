<cfcomponent displayname="ezAJAX_PopulateDebugScope" output="No" extends="userDefinedAJAXFunctions">
	...
	//				_html = _html & '<td id="btn_fetchDebug_#scopeName#.#ar[i]#" width="20%" valign="top" align="left" title="#_value#"><span class="buttonClass">"#scopeName#.#ar[i]#"</span></td>';
						...
						if ( (IsDefined("qryStruct.namedArgs.scopeName")) AND (Len(qryStruct.namedArgs.scopeName) gt 0) AND (NOT IsDefined("qryStruct.namedArgs.scopeMember")) ) {
							...
							try {
								...
								if (IsStruct(_datum)) {
									...
									htmlContent = URLEncodedFormat(ezCFDump(_datum, qryStruct.namedArgs.scopeMember, true));
