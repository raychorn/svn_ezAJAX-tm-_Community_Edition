				...
				if ( (IsDefined("Request.qryStruct.ezSessid")) AND (IsValid('UUID', Request.qryStruct.ezSessid)) ) {
					switch (Request.qryStruct.ezCFM) {
						case 'validateAJAXRuntimeLicense':
							...
							Request.commonCode.ezRegisterQueryFromAJAX(qObj); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
						break;
						...
						default:
							...
							if ( (NOT IsDefined("Request.qryData")) OR (NOT IsArray(Request.qryData)) OR (ArrayLen(Request.qryData) eq 0) ) {
								...
								QueryAddRow(qObj, 1);
								QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
								QuerySetCell(qObj, 'errorMsg', 'ezAJAXEngine Server Error - No Server Command implementor was detected or the Server Command implementor did not return a Query Object (#ArrayLen(Request.qryData)#) / (#IsDefined("Request.qryData")#).&nbsp;&nbsp;Double-check the userDefinedAJAXFunctions.cfc file to ensure you have implmented the command (#Request.qryStruct.ezCFM#) in "#CGI.SCRIPT_NAME#" and make sure it returns a Query Object.  This is a known problem with the FireFox 2.0 browser that may affect other Mozilla browsers or browsers that were built using the Mozilla browser engine.', qObj.recordCount);
