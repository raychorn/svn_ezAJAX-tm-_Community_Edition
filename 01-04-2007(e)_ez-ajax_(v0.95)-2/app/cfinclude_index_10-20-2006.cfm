...
// +++
		...
		function handle_onActivateTabsCallback(iTab) {
			iTab = (((typeof iTab) == const_string_symbol) ? iTab : iTab.toString());
			dTab = this.tabsCollection[iTab - 1];
			oDiv = _$(dTab);
			if (!!oDiv) {
	//			ezAlert('onActivateTabsCallback(' + iTab + ')');
				oAJAXEngine.doAJAX('performClickMenuTab', 'handlePerformClickMenuTab', 'iTab', iTab);
				return false;
			}
			return true;
		}
		
		function handlePerformClickMenuTab(qObj, nRecs, qStats, argsDict, qData1) {
			nRecs = ((nRecs != null) ? nRecs : -1);
			if (!!qStats) {
				if (!!argsDict) {
					...
					if (!!_iTAB) {
	//					oTabSystem1._activateTab(_iTAB);
					}
	...
	//					ezAlertError('Error - qData1 has an invalid value.');
		...
		function getTabbedContent() {
			...
			oTabSystem1.onActivateTabsCallback = handle_onActivateTabsCallback;
			...
			iContent = oTabSystem1.addContentForTab(iTabs.pop(), '<iframe id="iframe_siteMenuUsingDojo" width="870" frameborder="0" scrolling="No" src="app/siteMenu.cfm?mode=web20"></iframe>');
