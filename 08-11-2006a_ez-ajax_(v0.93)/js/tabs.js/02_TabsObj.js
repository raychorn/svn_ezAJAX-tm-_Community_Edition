...
ezTabsObj.hideSubTabs = function(oTabsObjId, iTab) {
	...
	if (oTabsObjId <= ezTabsObj.$.length) {
		...
		if (!!oTabsObj) {
			iTab = ((iTab != null) ? iTab : -1);
	//		alert('1.2 oTabsObj.isSubTabDismissable = [' + oTabsObj.isSubTabDismissable + ']');
...
ezTabsObj._isSubTabDismissable = function(oTabsObjId, bool) {
	...
	//		alert('1.1 oTabsObj.isSubTabDismissable = [' + oTabsObj.isSubTabDismissable + ']');
...
ezTabsObj.prototype = {
	...
	renderSubTabsFrom : function(iTab, anAR) {
		...
		html += '<table width="100%" cellpadding="-1" cellspacing="-1" onmouseover="this.border=0; ezTabsObj.isSubTabDismissable(' + this.id + ', true); return false;" onmouseout="this.border=1; ezTabsObj.hideSubTabs(' + this.id + ', ' + iTab + '); return false;">'; 
	...
	addSubTab : function(iTab, sTitle) {
		...
		if (iTab > -1) {
			...
			if ((typeof iSubTabs) == const_object_symbol) {
				...
				if (oOuter == null) {
					_html += '<div id="' + tabDivIdOuter + '" style="position: absolute; top: 0px; left: 0px; width: 0px; display: none; cursor:hand; cursor:pointer; z-index: 32767;">';
