...
<cfscript>
	...
	cfJavaScriptSourceCodeFName1 = "jce.js";
	cfJavaScriptSourceCodeFName2 = "jee.js";
...
<cfset makeNewRuntimeLicense = (NOT FileExists(ExpandPath('runtimeLicense.dat'))) AND 0>  <!--- (Request.commonCode.isServerLocal()) AND  --->
...
<cfif (FindNoCase("ezAJAXLogo.cfm", CGI.SCRIPT_NAME) eq 0)>
	...
	<cfif (makeNewRuntimeLicense)>
		<cfscript>
			try {
				Request.commonCode.writeRuntimeLicenseForEndDate(CreateDateTime(2099, 12, 31, 23, 59, 59), Request.commonCode.productName, CGI.SERVER_NAME, true);
			} catch (Any e) {
			};
		</cfscript>
	</cfif>
