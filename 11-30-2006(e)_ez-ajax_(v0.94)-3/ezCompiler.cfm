...
//	jsCodeList = '_js/433511201010924803.dat,_js/decontextmenu.js';
	...
	if ( (NOT Request.directoryError) AND (IsQuery(Request.qDir)) AND (Request.qDir.recordCount gt 0) ) {
		...
		if (NOT Request.dbError) {
			...
			if ( (NOT _directoryError2) AND (IsQuery(Request.qDirH2)) AND (Request.qDirH2.recordCount gt 0) ) {
				bool_recompileJavaScript = (DateCompare(Request.qDir2.DATELASTMODIFIED[1], Request.qDirH2.DATELASTMODIFIED[1]) gt 0);
			}
		}
...
<cfset bool_rebuildCfJavaScriptSourceCodeFile = (bool_recompileJavaScript) OR (NOT FileExists(fullyQualified_cfJavaScriptSourceCodeFName2))>
