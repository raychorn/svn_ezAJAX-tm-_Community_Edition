...
<cfset myAppNameCFM = ExpandPath("appname.cfm")>
...
<cfscript>
	...
	Request.cf_html_container_symbol = "ezajax_html_container";
	...
	err_ajaxCodeMsg = '';
	try {
		_prefix = findAJAXCodeCFC('ezAjaxCode');
		Request.commonCode = CreateObject("component", _prefix & ".ezAjaxCode");
	} catch(Any e) {
		err_ajaxCode = true;
		err_ajaxCodeMsg = '(1) The ezAjaxCode.cfc component has NOT been created.';
...
<cfif (NOT makeNewRuntimeLicense)>
	<cfscript>
		aStruct = Request.commonCode.readRuntimeLicenseFile(Request.commonCode.productName);
