...
ezAJAXEngine.remove$ = function(id) {
	...
	if ( (id > -1) && (id < ezAJAXEngine.$.length) ) {
		...
		if (!!instance) {
			ezAJAXEngine.$[id] = object_destructor(instance);
	...
	try {
		if (!!qObj) {
			...
			try {
				...
				nRecs = -1;
				_alertError(jsErrorExplainer(e, 'Error in abstract code handler upon return from ezAJAX(tm) Server Command. Reason: Unable to process the returned data structure due to a possible system error.'));
			} finally {
			}
			if (nRecs > 0) {
				...
				if (!!qData1) {
					...
					if (!!oParms) {
						...
						try {
							...
							_alertError(jsErrorExplainer(e, 'Error in user-defined code handled upon return from ezAJAX(tm) Server Command.'));
						} finally {
						}
					}
				}
		...
		_alertError(jsErrorExplainer(e, 'Error in abstract code handled upon return from ezAJAX(tm) Server Command.'));
	} finally {
	}
};
...
ezAJAXEngine.prototype = {
	...
	doAJAX : function(cmd, callBackFuncName, vararg_params) { 
	    ...
	    for (i += 2; i < n; i++) {
			args += arguments[i].toString().URLEncode();
			if (i < (n - 1)) {
				args += ',';
			}
		}
		doAJAX_func(this, cmd, callBackFuncName, args.toString());
	}, 
	register_ezAJAX_function : function(f) {
		this.stack_ezAJAX_functions.push(f);
	},
	handleNext_ezAJAX_function : function() {
		var f = this.stack_ezAJAX_functions.pop();
		if (!!f) { eval(f);	}
	},
	...
	showAJAXBegins : function(sMsg, sImage, imageHeight) {
		...
		if (!!cObj) {
			...
			if (!!frObj) {
				...
				cObj.style.top = '0px';
				cObj.style.left = (clientWid$() - this.ezAJAX_serverBusy_width - 1) + 'px';
	...
	html : function() {  
		...
		if (_$(this.ezAJAX_serverBusy_divName) == null) {
			html += '<div id="' + this.ezAJAX_serverBusy_divName + '" style="display: inline; position: absolute; top: 0px; left: ' + (clientWid$() - this.ezAJAX_serverBusy_width) + 'px"></div>';
		}
		this.myHTML = html;
		return html;
	},
