<cfoutput>
	...
	<!--
		...
		oAJAXEngine.createAJAXEngineCallback = function () { this.top = '400px'; /*_alert('oAJAXEngine.createAJAXEngineCallback()');*/ };
	...
	//		_alert('windowOnscrollCallback(' + top + ', ' + left + ')' + ', bool_isDebugPanelRepositionable = [' + bool_isDebugPanelRepositionable + ']');
		...
		function handleSampleAJAXCommand(qObj, nRecs, qStats, argsDict, qData1) {
	...
	_alert('handleSampleAJAXCommand :: qObj = [' + qObj + ']');
		...
		function simplerHandleSampleAJAXCommand(qObj, nRecs, qStats, argsDict, qData1) {
			...
			function searchForStatusRecs(_ri, _dict, _rowCntName) {
				if (aDict == -1) {
					aDict = DictObj.get$(_dict.asQueryString()); // This is the easy method for making a copy of a Dictionary Object instance.
				}
			};

			nRecs = ((nRecs != null) ? nRecs : -1);
			_alert('simplerHandleSampleAJAXCommand :: nRecs = [' + nRecs + ']');

			if (!!qStats) {
				_alert('simplerHandleSampleAJAXCommand :: qStats = [' + qStats + ']');

				if (!!argsDict) {
					_alert('simplerHandleSampleAJAXCommand :: argsDict = [' + argsDict + ']');

					if (!!qData1) {
						qData1.iterateRecObjs(searchForStatusRecs);
						_alert('simplerHandleSampleAJAXCommand :: qData1 = [' + qData1 + ']');
					} else {
						_alertError('Error - qData1 has an invalid value.');
					}
		
					DictObj.remove$(argsDict.id);
				} else {
					_alertError('Error - argsDict has an invalid value.');
				}
			} else {
				_alertError('Error - qStats has an invalid value.');
			}
		}
	// -->
	</script>
							...
							<table width="100%" align="left" border="0">
								<tr>
									<td align="left" valign="middle">
										<button class="buttonMenuClass<cfif (Request.commonCode.isBrowserFF() OR Request.commonCode.isBrowserNS())>FF</cfif>" onclick="oAJAXEngine.doAJAX('sampleAJAXCommand', 'handleSampleAJAXCommand', 'parm1', 'parm1-value', 'parm2', 'parm2-value', 'parm3', 'parm3-value', 'parm4', 'parm4-value'); return false;">Click to Test the exAJAX#cf_trademarkSymbol# Community Framework</button>
									</td>
								</tr>
								<tr>
									<td align="left" valign="middle">
										<button class="buttonMenuClass<cfif (Request.commonCode.isBrowserFF() OR Request.commonCode.isBrowserNS())>FF</cfif>" onclick="oAJAXEngine.doAJAX('sampleAJAXCommand', 'simplerHandleSampleAJAXCommand', 'parm1', 'parm1-value', 'parm2', 'parm2-value', 'parm3', 'parm3-value', 'parm4', 'parm4-value'); return false;">Click to Test the exAJAX#cf_trademarkSymbol# Community Framework (simpler)</button>
									</td>
								</tr>
							</table>
						</td>
