...
<cfoutput>
	...
	//			dObj.innerHTML = '&nbsp;<b>Extra Menu Content</b>';
		...
		function ezWindowOnscrollCallback(top, left) {
			...
			if (bool_isDebugPanelRepositionable) {
				...
				var dObj = $(const_div_floating_debug_menu);
				if (!!dObj) {
					dObj.style.display = (((window.location.href.toUpperCase().indexOf('.contentopia.net') > -1) || (window.location.href.toUpperCase().indexOf('.ez-ajax.com') > -1)) ? const_none_style : const_inline_style);
				}
			}
	...
	<cfscript>
		...
		_urlWaferAnalysis = Request.commonCode.ezClusterizeURL('http://#CGI.SERVER_NAME#/' & 'samples/semiconductors/wafer-analysis/index.cfm') & '?sessID=#Session.sessID#';
...
//	ezAlert('qObj = [' + qObj + ']');
	...
	//		html += '</tr>';
			...
			html += '<span class="menux11StatusBoldClass">What is ezAJAX' + js_trademarkSymbol + '</span>';
			html += '</td>';
			html += '</tr>';
			html += '</table>';
			html += '</td>';
