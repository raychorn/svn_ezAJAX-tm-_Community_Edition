...
<cfif (NOT debugJavaScriptPackager)>
		...
		<div id="#Request.cf_div_floating_debug_menu#" style="display: block; height: 25px; background-color: ##80FFFF; border: thin solid Black; z-index: 65535;">
			<button id="btn_fetchDebugMenu" class="buttonClass" onclick="try { oAJAXEngine.doAJAX('ezAJAX_PopulateDebugPanel', 'handle_ezAJAXPopulateDebugPanel'); } catch (e) { ezAlertError(ezErrorExplainer(e)); }; return false;" title="Click this button to begin using the Debug menu.">[Open ezAJAX Debug Menu]</button>
		</div>
		...
		<cfif 0>
			<div id="writersXML" name="writersXML" style="position: absolute; top: 100px; left: 200px;"><cfdump var="#Request.loaderCode#" label="Request.loaderCode" expand="No"></div>
		</cfif>
						...
						if (!!argsDict) {
							if (!!qData1) {
								...
								if (_CONTENT.length > 0) {
									...
									ezAlertError('nRecs = [' + nRecs + ']');
									ezAlertError('qStats = [' + qStats + ']');
									ezAlertError('argsDict = [' + argsDict + ']');
									ezAlertError('qData1 = [' + qData1 + ']');
									ezAlertError('qObj = [' + qObj + ']');
								}
							} else {
