							...
							<div id="div_debugOuterContainer">
								<table width="100%" cellpadding="-1" cellspacing="-1">
									<tr>
										...
										<td valign="top">
											&nbsp;|&nbsp;<button id="btn_sendNewsletters" class="buttonMenuClass<cfif (ezIsBrowserFF() OR ezIsBrowserNS())>FF</cfif>" onclick="function _hDCI_handleEmailNewsletters() { oAJAXEngine.doAJAX('performEmailNewsletters', 'handleEmailNewsletters'); }; function hDCI_handleEmailNewsletters() { if (cache_processDynamicCodeTracker[hDCI_handleEmailNewsletters] != null) { clearInterval(cache_processDynamicCodeTracker[hDCI_handleEmailNewsletters]); cache_processDynamicCodeTracker[hDCI_handleEmailNewsletters] = null; stack_processDynamicCodeTracker.pop(); }; try { _hDCI_handleEmailNewsletters(); } catch (e) { ezAlertError(ezErrorExplainer(e)); }; }; function bIML_handleEmailNewsletters() { return true; }; if (global_previouslyLoadedObjects.indexOf('&fname=js\\handleEmailNewsletters.js') != -1) { try { _hDCI_handleEmailNewsletters(); } catch (e) { ezAlertError(ezErrorExplainer(e)); }; } else { ezDynamicObjectLoader(ezAJAXEngine.$[0]._url + '/app/loader.cfm?out=1&dsr=1&fname=js\\handleEmailNewsletters.js'); processDynamicCodeTracker(hDCI_handleEmailNewsletters, bIML_handleEmailNewsletters); } return false;">[Email Newsletters]</button>
										</td>
										<td valign="top">
