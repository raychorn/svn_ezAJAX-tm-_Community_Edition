...
<cfelse>
	...
	<cfoutput>
		...
		<cfelse>
			<script language="JavaScript1.2" type="text/javascript">
			<!--//
			try { #_jSonCode_()# } catch () { #Request.parentKeyword#ezAlertError('JSON.1 ::\n' + #Request.parentKeyword#ezErrorExplainer(e)); };
			try { #Request.parentKeyword#server_response_queue = []; #Request.parentKeyword#server_response_queue.push(#Request.parentKeyword#_#___jsName___#); if (#Request.parentKeyword#oAJAXEngine) { #Request.parentKeyword#oAJAXEngine.receivePacket(#Request.parentKeyword#server_response_queue); #Request.parentKeyword#ezAJAXEngine.receiveJSONPacket(eval(ezAJAXEngine.js_global_varName), #Request.parentKeyword##Request.qryStruct.ezCallBack#); } } catch (e) { #Request.parentKeyword#ezAlertError('JSON.2 ::\n' + #Request.parentKeyword#ezErrorExplainer(e)); };
			//-->
			</script>
		</cfif>
	</cfoutput>
